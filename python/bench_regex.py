...
`regex` (https://pypi.org/project/regex/) is Python's de-facto Unicode-aware
regex library — the one most BPE / tokenizer code reaches for, since stdlib
`re` has no `\p{}` property syntax. It is a backtracking engine that supports
look-around, `\p{...}`, inline flags and possessive quantifiers, so it runs
the verbatim GPT-4 `tokenizer` pattern (unlike stdlib `re`, which REJECTs it).

Same CSV schema, workloads, corpus, sizing and pathological-in-a-child
methodology as the sibling stdlib `re` harness (`bench.py`); only the engine
column differs (`python-regex`). run_all.sh installs `regex` into a local
venv (idempotent) so the rest of the tree / global site-packages are
untouched.
...
WORKLOADS = [
    ...
    # Feature-heavy: the PyPI `regex` module supports all of these (backref,
    # look-behind, `\p{}`, atomic groups) and runs them verbatim.
    ...
    # GPT-4 cl100k_base pre-tokenizer — verbatim. The `regex` module supports
    # `(?i:)`, `\p{L}`/`\p{N}`, `(?!\S)` and possessive `?+`/`++`, so this
    # runs unchanged; its match counts are gate-checked against zeetah,
    # fancy-regex and .NET (all four agree: 212 / 1755 / 7116).
...
def bench_one(wl_id, pattern, text, byte_len):
    ...
    for _ in range(50):
        ...
        try:
            ...
            print(f"python-regex,{wl_id},{byte_len},0,-1,-1,0.00,-1,REJECTED")
            return
    ...
    print(f"python-regex,{wl_id},{byte_len},{iters},{compile_min},{median},"
...
def run_pathological():
    ...
    try:
        ...
        print(f"python-regex,pathological,{PATHO_INPUT_LEN},1,-1,{dt},"
              f"{tput:.2f},0,ok")
    except subprocess.TimeoutExpired:
        print(f"python-regex,pathological,{PATHO_INPUT_LEN},0,-1,-1,0.00,-1,TIMEOUT")


def main():
    # Child mode: just attempt the catastrophic match (parent times/limits us).
    if os.environ.get("BENCH_PATHO_CHILD") == "1":
        rx = regex.compile(PATHO_PATTERN)
